...
sgx_oram::Oram::Oram(const cxxopts::ParseResult& result)
  ...
  const uint32_t bucket_number =
      std::ceil(all_blocks * 1.0 / bucket_size);
...
void sgx_oram::Oram::run_test(void) {
  ...
  for (uint32_t r = 0; r < round; r++) {
    for (uint32_t i = 0; i < block_number; i++) {
      ...
      try {
        Block b(true);
        auto res = oram_access(0, i, b, 0, position_map[i].bid_cur);
...
// FIXME: Write will be done multiple times???
// FIXME: Is the variable data useless?
sgx_oram::Block sgx_oram::Oram::oram_access(const bool& op, const uint32_t& u,
                                            Block& data,
                                            const uint32_t& level_cur,
                                            const uint32_t& bid) {
  // Fetch the slot given the bid and the level.
  ...
  if (u != UINT_MAX) {
    LOG(plog::error) << bucket_fullness[nbid];
  ...
  if (level_cur == level - 1) {
    ...
    if (u != UINT_MAX) {
      ...
      if (slot.dummy_number == 0) {
        LOG(plog::error) << bucket_fullness[slot.range.first];
