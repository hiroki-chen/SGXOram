        ...
        for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow) {
            if (std::distance(mData.begin(), itRow) > mLabelParams.mColumnNameIdx) {
                if (columnIdx < static_cast<ssize_t>(itRow->size())) {
                    ...
                    const std::string errStr = "requested column index " + std::to_string(columnIdx - (mLabelParams.mRowNameIdx + 1)) + " >= " + std::to_string(itRow->size() - (mLabelParams.mRowNameIdx + 1)) + " (number of columns on row index " + std::to_string(std::distance(mData.begin(), itRow) - (mLabelParams.mColumnNameIdx + 1)) + ")";
