...
{
	ms_ocall_panic_and_flush_t* ms = SGX_CAST(ms_ocall_panic_and_flush_t*, pms);
	ocall_panic_and_flush(ms->ms_reason);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL enclave_ocall_flush_log(void* pms)
{
