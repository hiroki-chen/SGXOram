...
TPPartitionORAMInstance::TPPartitionORAMInstance(
    const uint32_t& numofReals, size_t& real_ind, const uint32_t& PID,
    std::string* s_buffer, Position* p_map, MongoConnector* Conn)
    : PartitionORAMInstance(), PID(PID), pos_map(p_map) {
  n_levels = (uint32_t)(log((double)numofReals) / log(2.0)) + 1;
  if (numofReals < 6)
    capacity = (uint32_t)ceil(small_capacity_parameter * numofReals);
  else
    capacity = (uint32_t)ceil(capacity_parameter * numofReals);
  ...
  for (uint32_t i = 0; i < n_levels - 1; ++i) {
    for (uint32_t j = 0; j < (1 << (i + 1)); ++j) {
      sbuffer[j] =
          Util::generate_random_block(B - AES::BLOCKSIZE - sizeof(uint32_t));
      int32_t dummyID = -1;
  ...
  for (uint32_t k = 0; k < numofReals; k++) {
    sbuffer[k] =
        Util::generate_random_block(B - AES::BLOCKSIZE - 2 * sizeof(uint32_t));
  ...
  for (uint32_t k = 0; k < top_level_len; k++) {
    ...
    if (bID2 < 0) {
      ...
      int32_t dummyID2 = -1;
      sbuffer[k] = std::string((const char*)(&dummyID2), sizeof(uint32_t)) +
    ...
    std::string cipher;
    Util::aes_encrypt(sbuffer[k],
                      &(key[(n_levels - 1) * AES::DEFAULT_KEYLENGTH]), cipher);
    sbuffer[k] = cipher;
  }
  ...
  for (uint32_t l = 0; l < n_levels; l++) {
    if (filled[l]) {
      if (block_id != -1 && pos_map[block_id].level == l) {
        ...
        readBlock(l, dummy[(1 << l) - 1 + (counter % (1 << l))]);
    }
  }
...
void TPPartitionORAMInstance::put(const int32_t& block_id,
  ...
  if (level != -1) {
    for (uint32_t l = 0; l <= level; l++) {
      ...
      for (size_t i = begin_id; i < begin_id + length; i++) {
        ...
        Util::aes_decrypt(sbuffer[i], &(key[l * AES::DEFAULT_KEYLENGTH]),
                          plainBlocks);
        sbuffer[i] = plainBlocks;
        memcpy(&dID, sbuffer[i].c_str(), sizeof(uint32_t));
        if (pos_map[dID].part == PID && dID != -1 && dID != block_id &&
            pos_map[dID].level == l) {
  ...
  for (uint32_t i = 0; i < sbuffer_len; i++) {
    ...
    if (bID2 < 0) {
      ...
      int32_t s_id = -1;
      sbuffer[i] = std::string((const char*)(&s_id), sizeof(uint32_t)) +
    ...
    std::string cipher;
    Util::aes_encrypt(sbuffer[i], &(key[level * AES::DEFAULT_KEYLENGTH]),
                      cipher);
    sbuffer[i] = cipher;
  }
  std::cout << "put5\n";
  ...
  std::cout << "put6\n";
  for (size_t i = 0; i < level; i++) filled[i] = false;
  filled[level] = true;
  counter = (counter + 1) % (1 << (n_levels - 1));
}

std::string TPPartitionORAMInstance::readBlock(uint32_t level,
                                               uint32_t offset) {
...
void TPPartitionORAMInstance::fetchLevel(uint32_t level, std::string* sbuffer,
                                         uint32_t beginIndex, size_t& length) {
...
void TPPartitionORAMInstance::loadLevel(uint32_t level, std::string* sbuffer,
                                        size_t length, bool insert) {
  uint32_t begin = (uint32_t)(1 << (level + 1)) - 2;

  if (insert)
    conn->insert(sbuffer, begin, length, ns);
  else
    conn->update(sbuffer, begin, length, ns);
