...
std::string PartitionORAM<T>::get(const std::string & key) {
    std::string res;
    uint32_t int_key = std::stoul(key);
...
void PartitionORAM<T>::evict(uint32_t pid) {
  std::cout << "evict\n";
    if (!slots[pid].empty()) {
        ...
        dummy = dID + dummy;
        partitions[pid] -> put(-1, dummy); // Error.
    }
    std::cout << "evict2\n";
...
void PartitionORAM<T>::access(char op, uint32_t block_id, std::string & data) {
  ...
  std::cout << "access2" << std::endl;
    ...
    std::cout << "access4" << std::endl;

    evict(p);
    std::cout << "access5" << std::endl;

    counter++;
    sequentialEvict();
    std::cout << "access6" << std::endl;
}
