...
template <size_t N, class T = Bitset2::detail::select_base_t<N>,
          class Enabled = void>
class bitset2;
 ...
 public:
  ...
  class reference {
   ...
   public:
    ...
    operator bool() const noexcept { return m_ptr->test_noexcept(m_bit); }
  ...
  bitset2& operator=(bitset2 const&) noexcept = default;

  bitset2& operator=(bitset2&&) noexcept = default;

  explicit bitset2(const std::bitset<N>& bs) noexcept
      : detail::bitset2_impl<N, T>(bs) {}

  explicit constexpr bitset2(ULLONG_t v) noexcept
      : detail::bitset2_impl<N, T>(v) {}

  template <size_t n, class Tsrc>
  explicit constexpr bitset2(std::array<Tsrc, n> const& value) noexcept
  ...
  reference operator[](size_t bit) noexcept { return reference(this, bit); }

  bitset2& operator<<=(size_t n_shift) noexcept {
  ...
  bitset2& operator|=(bitset2 const& v2) noexcept {
    detail::array_funcs<bitset2::n_array, T>().bitwise_or_assgn(
        this->get_data(), v2.data());
    return *this;
  }

  bitset2& operator&=(bitset2 const& v2) noexcept {
    detail::array_funcs<bitset2::n_array, T>().bitwise_and_assgn(
        this->get_data(), v2.data());
    return *this;
  }

  bitset2& operator^=(bitset2 const& v2) noexcept {
    detail::array_funcs<bitset2::n_array, T>().bitwise_xor_assgn(
        this->get_data(), v2.data());
    return *this;
  }

  /// Computes the set difference, i.e. *this &= ~v2
  bitset2& difference(bitset2 const& v2) noexcept {
    detail::array_funcs<bitset2::n_array, T>().bitwise_setdiff_assgn(
        this->get_data(), v2.data());
    return *this;
  }

  bitset2& set() noexcept {
    detail::bitset2_impl<N, T>::set();
    return *this;
  }

  bitset2& set(size_t bit, bool value = true) {
    detail::bitset2_impl<N, T>::set(bit, value);
    return *this;
  }

  bitset2& reset() noexcept {
    detail::bitset2_impl<N, T>::reset();
    return *this;
  }

  bitset2& reset(size_t bit) {
  ...
  bool test_set(size_t bit, bool value = true) {
    return detail::bitset2_impl<N, T>::test_set(bit, value);
  }

  bitset2& flip() noexcept {
    detail::bitset2_impl<N, T>::flip();
    return *this;
  }

  bitset2& flip(size_t bit) {
  ...
  std::basic_string<CharT, Traits, Allocator> to_hex_string(
      hex_params<CharT, Traits, Allocator> const& params =
...
template <size_t N, class T>
constexpr bitset2<N, T> rotate_left(bitset2<N, T> const& bs,
                                    size_t n_rot) noexcept {
  return bitset2<N, T>(detail::array_ops<N, T>(n_rot).rotate_left(bs.data()));
}

template <size_t N, class T>
constexpr bitset2<N, T> rotate_right(bitset2<N, T> const& bs,
                                     size_t n_rot) noexcept {
  return bitset2<N, T>(
      detail::array_ops<N, T>(N - (n_rot % N)).rotate_left(bs.data()));
}

/// Computes the set difference, i.e. bs1 & ~bs2
template <size_t N, class T>
constexpr bitset2<N, T> difference(bitset2<N, T> const& bs1,
                                   bitset2<N, T> const& bs2) noexcept {
  return bitset2<N, T>(
      detail::array_funcs<bitset2<N, T>::n_array, T>().bitwise_setdiff(
          bs1.data(), bs2.data()));
}
...
constexpr bitset2<N, T1> convert_to(bitset2<M, T2> const& bs) noexcept {
  using a2a = detail::array2array<bitset2<N, T1>::n_array,
                                  bitset2<M, T2>::n_array, T1, T2>;
  return bitset2<N, T1>(
      a2a()(detail::bit_chars<N, T1>::hgh_bit_pattern, bs.data()));
}
...
constexpr bool zip_fold_and(bitset2<N, T> const& bs1, bitset2<N, T> const& bs2,
  ...
  return detail::array_funcs<bitset2<N, T>::n_array, T>().zip_fold_and(
      bs1.data(), bs2.data(), f);
}
...
constexpr bool zip_fold_or(bitset2<N, T> const& bs1, bitset2<N, T> const& bs2,
                           F f) noexcept(noexcept(f(T(0), T(0)))) {
  return detail::array_funcs<bitset2<N, T>::n_array, T>().zip_fold_or(
      bs1.data(), bs2.data(), f);
}
...
std::basic_ostream<CharT, Traits>& operator<<(
    std::basic_ostream<CharT, Traits>& os, Bitset2::bitset2<N, T> const& x) {
  for (size_t ct = N; ct > 0;) {
    --ct;
    os << (x[ct] ? "1" : "0");
  }
...
std::basic_istream<CharT, Traits>& operator>>(
    std::basic_istream<CharT, Traits>& is, Bitset2::bitset2<N, T>& x) {
  std::bitset<N> bs;
  is >> bs;
...
/// Shift left
template <size_t N, class T>
constexpr Bitset2::bitset2<N, T> operator<<(Bitset2::bitset2<N, T> const& bs,
                                            size_t n_shift) noexcept {
  return Bitset2::bitset2<N, T>(
      Bitset2::detail::array_ops<N, T>(n_shift).shift_left(bs.data()));
}
...
constexpr Bitset2::bitset2<N, T> operator>>(Bitset2::bitset2<N, T> const& bs,
                                            size_t n_shift) noexcept {
  return Bitset2::bitset2<N, T>(
      Bitset2::detail::array_ops<N, T>(n_shift).shift_right(bs.data()));
}

template <size_t N, class T>
constexpr Bitset2::bitset2<N, T> operator|(
    Bitset2::bitset2<N, T> const& bs1,
...
struct hash<Bitset2::bitset2<N, T> > {
 private:
  enum : size_t { n_array = Bitset2::detail::bitset2_impl<N, T>::n_array };
